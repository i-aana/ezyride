...
interface CalendarPriceData {
  date: string;
  price: number;
  status: 'available' | 'booked' | 'blocked' | 'request';
}
...
const Hero: React.FC<HeroProps> = ({ 
  ...
  const [calendarData, setCalendarData] = useState<CalendarPriceData[]>([]);
  ...
  // Helper function to check if date exists in database and is available
  const getDateInfo = (date: Date): CalendarPriceData | null => {
    const dateStr = formatDateForDatabase(date);
    return calendarData.find(item => item.date === dateStr) || null;
  };

  // Check if date should be disabled for pickup
  const isPickupDateDisabled = (date: Date): boolean => {
    const dateInfo = getDateInfo(date);
    // Disable if date doesn't exist in DB or is booked/blocked
    return !dateInfo || dateInfo.status === 'booked' || dateInfo.status === 'blocked';
  };

  // Find the maximum valid return date for a given pickup date
  const getMaxReturnDate = (pickupDate: Date): Date => {
    let currentDate = new Date(pickupDate);
    
    while (true) {
      const dateInfo = getDateInfo(currentDate);
      // If current date doesn't exist in DB or is booked/blocked, return previous date
      if (!dateInfo || dateInfo.status === 'booked' || dateInfo.status === 'blocked') {
      ...
      // Safety check to prevent infinite loop (limit to 1 year ahead)
      if (currentDate.getTime() > pickupDate.getTime() + 365 * 24 * 60 * 60 * 1000) {
        break;
      }
    }
  ...
  // Check if date should be disabled for return
  const isReturnDateDisabled = (date: Date): boolean => {
    if (!dateRange.pickupDate) return true;
    
    // Return date must be after pickup date
    if (date <= dateRange.pickupDate) return true;
    ...
    // Check if all dates in the range from pickup to this date are available
    const maxReturnDate = getMaxReturnDate(dateRange.pickupDate);
    return date > maxReturnDate;
  };

  // Validate if a date range is continuous and available
  const isRangeContinuous = (startDate: Date, endDate: Date): boolean => {
  ...
  const handlePickupDateChange = (date: Date | null) => {
    ...
    // Clear return date if it's no longer valid
    if (date && dateRange.returnDate) {
      if (!isRangeContinuous(date, dateRange.returnDate) || dateRange.returnDate <= date) {
        onDateChange('returnDate', null);
  ...
  const handleReturnDateChange = (date: Date | null) => {
    ...
    if (date && dateRange.pickupDate) {
      if (!isRangeContinuous(dateRange.pickupDate, date)) {
        // Don't allow invalid range selection
        return;
      }
    }
  ...
  return (
    <div className="relative min-h-screen flex">
      {showMessage && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 bg-white border border-gray-200 rounded-lg shadow-lg px-6 py-4 max-w-md">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-900">
                Please select both pickup and return dates to check availability.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="p-6 space-y-4 flex-grow flex flex-col justify-between">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <DatePicker
                    selected={dateRange.returnDate}
                    onChange={handleReturnDateChange}
                    minDate={dateRange.pickupDate ? getNextDay(dateRange.pickupDate) : new Date()}
                    maxDate={dateRange.pickupDate ? getMaxReturnDate(dateRange.pickupDate) : undefined}
                    filterDate={(date) => !isReturnDateDisabled(date)}
                    dateFormat="yy/MM/dd"
                    placeholderText="Select return date"
                    customInput={<CustomInput />}
                    popperPlacement="bottom-start"
                    popperModifiers={popperModifiers}
                    disabled={!dateRange.pickupDate}
                    shouldCloseOnSelect={true}
                  />
                </div>
              </div>
              ...
              {dateRange.pickupDate && (
                <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
                  <p><strong>Pickup:</strong> {formatDate(dateRange.pickupDate)}</p>
                  {dateRange.returnDate && (
                    <p><strong>Return:</strong> {formatDate(dateRange.returnDate)}</p>
                  )}
                  {dateRange.pickupDate && !dateRange.returnDate && (
                    <p className="text-blue-600 mt-1">
                      Available return dates: {formatDate(getNextDay(dateRange.pickupDate))} - {formatDate(getMaxReturnDate(dateRange.pickupDate))}
                    </p>
                  )}
                </div>
              )}
